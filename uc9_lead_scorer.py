"""
OrangeTree Global — AI Lead Scorer (UC9)
Powered by Claude API (Anthropic)
Run: streamlit run uc9_lead_scorer.py
"""

import streamlit as st
import anthropic
import io
import json

LOGO_B64 = ""
LOGO_HTML = '<img src="data:image/svg+xml;base64,' + LOGO_B64 + '" style="width:100%;max-width:220px;display:block;margin-bottom:4px;">'

st.set_page_config(
    page_title="Lead Scorer | OrangeTree Global",
    page_icon="🌳",
    layout="wide",
)

st.markdown("""
<style>
    .section-header {
        border-left: 4px solid #F36621;
        padding: 0.3rem 0.8rem;
        background: #fff3ee;
        border-radius: 0 4px 4px 0;
        margin: 1.2rem 0 0.6rem 0;
        font-weight: 600;
    }
    .hot-badge { color: #c0392b; font-weight: 700; }
    .warm-badge { color: #e67e22; font-weight: 700; }
    .cold-badge { color: #7f8c8d; font-weight: 700; }
</style>
""", unsafe_allow_html=True)

st.markdown(
    f'<img src="data:image/svg+xml;base64,{LOGO_B64}" style="width:280px;max-width:100%;display:block;margin-bottom:12px;">',
    unsafe_allow_html=True
)
st.markdown("## AI Lead Scorer")
st.markdown("Upload your leads CSV — Claude scores each lead as Hot / Warm / Cold with a follow-up note and priority action.")
st.divider()

with st.sidebar:
    st.markdown(LOGO_HTML, unsafe_allow_html=True)
    st.markdown("---")
    api_key = st.text_input("Anthropic API Key", type="password", help="Get from console.anthropic.com")
    model = st.selectbox("Model", ["claude-opus-4-8", "claude-sonnet-4-6", "claude-haiku-4-5-20251001"], index=2,
        help="Haiku is fast and cost-effective for batch lead scoring")
    st.markdown("---")
    st.markdown("**Scoring criteria**")
    business_context = st.text_area(
        "Describe your ideal customer",
        placeholder="E.g. SME owners in India with 20-200 employees, interested in AI training, budget above ₹2L",
        height=100
    )
    st.markdown("---")
    st.info("UC9 of OrangeTree's AI SME Suite. Powered by Claude API.", icon="ℹ️")

tab_upload, tab_single = st.tabs(["Batch Scoring (CSV)", "Score Single Lead"])

with tab_upload:
    st.markdown('<div class="section-header">Upload Leads CSV</div>', unsafe_allow_html=True)
    uploaded = st.file_uploader("Upload leads CSV", type=["csv"], label_visibility="collapsed")

    if uploaded:
        try:
            import pandas as pd
            df = pd.read_csv(uploaded)
            st.dataframe(df.head(8), use_container_width=True)
            st.caption(f"{len(df)} leads | Columns: {list(df.columns)}")

            score_btn = st.button("Score All Leads", type="primary", use_container_width=True)

            if score_btn:
                if not api_key:
                    st.error("Enter your Anthropic API key in the sidebar.")
                    st.stop()

                progress = st.progress(0, text="Scoring leads...")
                client = anthropic.Anthropic(api_key=api_key)
                results = []

                for i, row in df.iterrows():
                    lead_str = " | ".join([f"{col}: {val}" for col, val in row.items() if str(val) != "nan"])

                    icp = business_context if business_context.strip() else "SME businesses interested in AI and data training"

                    prompt = f"""Score this sales lead for: {icp}

Lead: {lead_str}

Return JSON only:
{{
  "score": "Hot" or "Warm" or "Cold",
  "reason": "One sentence why",
  "follow_up": "Specific next action for sales team",
  "priority": 1-10
}}"""

                    resp = client.messages.create(
                        model=model,
                        max_tokens=300,
                        messages=[{"role": "user", "content": prompt}]
                    )

                    raw = resp.content[0].text.strip()
                    if "```" in raw:
                        raw = raw.split("```")[1]
                        if raw.startswith("json"):
                            raw = raw[4:]
                    try:
                        scored = json.loads(raw)
                    except Exception:
                        scored = {"score": "Warm", "reason": "Could not parse", "follow_up": "Manual review needed", "priority": 5}

                    results.append(scored)
                    progress.progress((i + 1) / len(df), text=f"Scored {i+1}/{len(df)} leads...")

                scored_df = df.copy()
                scored_df["Score"] = [r.get("score", "") for r in results]
                scored_df["Priority"] = [r.get("priority", 5) for r in results]
                scored_df["Reason"] = [r.get("reason", "") for r in results]
                scored_df["Follow-up Action"] = [r.get("follow_up", "") for r in results]
                scored_df = scored_df.sort_values("Priority", ascending=False)

                st.divider()
                st.markdown('<div class="section-header">Scored Leads</div>', unsafe_allow_html=True)

                hot = scored_df[scored_df["Score"] == "Hot"]
                warm = scored_df[scored_df["Score"] == "Warm"]
                cold = scored_df[scored_df["Score"] == "Cold"]

                col1, col2, col3 = st.columns(3)
                col1.metric("Hot leads", len(hot))
                col2.metric("Warm leads", len(warm))
                col3.metric("Cold leads", len(cold))

                st.dataframe(scored_df, use_container_width=True)

                csv_out = io.StringIO()
                scored_df.to_csv(csv_out, index=False)
                st.download_button(
                    "Download Scored Leads CSV",
                    data=csv_out.getvalue(),
                    file_name="scored_leads.csv",
                    mime="text/csv",
                    use_container_width=True
                )

        except Exception as e:
            st.error(f"Error: {e}")
    else:
        st.info("Upload a CSV with lead data. Any columns work — name, email, company, query, source, etc.", icon="ℹ️")
        with st.expander("Expected CSV format"):
            st.code("""Name,Company,Email,Query,Source,Company Size
Rahul Sharma,TechCorp,rahul@techcorp.com,Interested in AI training for 50 staff,LinkedIn,50-200
Priya Patel,StartupXYZ,priya@startup.com,Just browsing,Website,1-10
...""", language="text")

with tab_single:
    st.markdown('<div class="section-header">Score a Single Lead</div>', unsafe_allow_html=True)

    col_a, col_b = st.columns(2)
    with col_a:
        name = st.text_input("Name")
        company = st.text_input("Company")
        company_size = st.selectbox("Company size", ["1-10", "11-50", "51-200", "201-500", "500+"])
    with col_b:
        source = st.text_input("Lead source", placeholder="LinkedIn, referral, website...")
        budget = st.text_input("Budget indication", placeholder="₹1L, ₹5L, unknown...")
        query = st.text_area("Lead query / notes", height=80, placeholder="What did they ask about?")

    if st.button("Score This Lead", type="primary", use_container_width=True):
        if not api_key:
            st.error("Enter your Anthropic API key in the sidebar.")
            st.stop()

        lead_str = f"Name: {name} | Company: {company} | Size: {company_size} | Source: {source} | Budget: {budget} | Query: {query}"
        icp = business_context if business_context.strip() else "SME businesses interested in AI and data training"

        with st.spinner("Scoring lead..."):
            client = anthropic.Anthropic(api_key=api_key)
            prompt = f"""Score this sales lead for: {icp}

Lead: {lead_str}

Return JSON only:
{{
  "score": "Hot" or "Warm" or "Cold",
  "reason": "One sentence why",
  "follow_up": "Specific next action for the sales team",
  "priority": 1-10
}}"""

            resp = client.messages.create(
                model=model,
                max_tokens=300,
                messages=[{"role": "user", "content": prompt}]
            )

            raw = resp.content[0].text.strip()
            if "```" in raw:
                raw = raw.split("```")[1]
                if raw.startswith("json"):
                    raw = raw[4:]
            try:
                scored = json.loads(raw)
                score = scored.get("score", "Warm")
                color = "#c0392b" if score == "Hot" else "#e67e22" if score == "Warm" else "#7f8c8d"
                st.markdown(
                    f'<div style="background:#fff3ee;border-left:4px solid {color};padding:1rem;border-radius:4px;margin-top:1rem;">' +
                    f'<div style="font-size:1.4rem;font-weight:700;color:{color}">{score}</div>' +
                    f'<div style="margin-top:0.5rem"><b>Why:</b> {scored.get("reason","")}</div>' +
                    f'<div style="margin-top:0.5rem"><b>Next action:</b> {scored.get("follow_up","")}</div>' +
                    f'<div style="margin-top:0.5rem"><b>Priority score:</b> {scored.get("priority","")}/10</div>' +
                    '</div>',
                    unsafe_allow_html=True
                )
            except Exception:
                st.error("Could not parse score. Raw response:")
                st.code(raw)

st.caption("OrangeTree Global | AI SME Suite — UC9: Lead Scorer")
